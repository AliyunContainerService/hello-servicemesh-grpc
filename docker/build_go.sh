...
cd ../docker
docker build -f grpc-server-go.dockerfile -t registry.cn-beijing.aliyuncs.com/asm_repo/grpc_server_go:1.0.0 .
rm -rf proto_server
echo
...
cd ../docker
docker build -f grpc-client-go.dockerfile -t registry.cn-beijing.aliyuncs.com/asm_repo/grpc_client_go:1.0.0 .
rm -rf proto_client
echo