...
# k exec "$client_node_pod" -c grpc-client-node -n grpc-tracing -- node mesh_client.js grpc-server-svc1.grpc-tracing.svc.cluster.local 7777
# echo "Test from ingressgateway:"
# docker exec -it "$client_node_container" node mesh_client.js ${INGRESS_IP} 7777

echo "Test in a loop:"
for i in {1..100}; do
  docker exec -e GRPC_SERVER="${INGRESS_IP}" -it "$client_node_container" node mesh_client.js ${INGRESS_IP} 7777
done